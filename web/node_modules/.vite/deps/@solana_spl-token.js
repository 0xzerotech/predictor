...
} from "./chunk-NFKYUAKB.js";
import "./chunk-TKAGYST5.js";
...
var splDiscriminate = (discriminator, length = 8) => {
  const digest = (0, import_crypto.createHash)("sha256").update(discriminator).digest();
...
function getPublicKeyEncoder() {
  return mapEncoder(getBytesEncoder({ size: 32 }), (publicKey2) => publicKey2.toBytes());
}
...
var getU32Codec2 = (config = {}) => combineCodec2(getU32Encoder2(config), getU32Decoder2(config));
...
function getBooleanEncoder(config = {}) {
  return transformEncoder(config.size ?? getU8Encoder2(), (value) => value ? 1 : 0);
}
...
function assertValidVariantIndex(variants, index) {
  if (typeof variants[index] === "undefined") {
    throw new SolanaError2(SOLANA_ERROR__CODECS__UNION_VARIANT_OUT_OF_RANGE, {
...
function getDiscriminatedUnionEncoder(variants, config = {}) {
  ...
  return getUnionEncoder(
    variants.map(
      ([, variant], index) => transformEncoder(getTupleEncoder2([prefix, variant]), (value) => [index, value])
    ),
...
function getVariantDiscriminator(variants, discriminatorValue) {
  ...
  if (discriminator < 0) {
    throw new SolanaError2(SOLANA_ERROR__CODECS__INVALID_DISCRIMINATED_UNION_VARIANT, {
...
function getOptionEncoder(item, config = {}) {
  ...
  return getUnionEncoder(
    [
      transformEncoder(getTupleEncoder2([prefix, noneValue]), (_value) => [
        false,
        void 0
      ]),
      transformEncoder(getTupleEncoder2([prefix, item]), (value) => [
        true,
...
var getFieldCodec = () => [
  ...
  ["Symbol", getUnitCodec()],
  ["Uri", getUnitCodec()],
  ["Key", getStructCodec2([["value", getTupleCodec([addCodecSizePrefix(getUtf8Codec(), getU32Codec2())])]])]
];
...
function getInstructionEncoder2(discriminator, dataEncoder) {
  return transformEncoder(getTupleEncoder2([getBytesEncoder2(), dataEncoder]), (data) => [
    discriminator,
    data
  ]);
}
function getPublicKeyEncoder2() {
  return transformEncoder(fixEncoderSize(getBytesEncoder2(), 32), (publicKey2) => publicKey2.toBytes());
}
...
function getStringCodec() {
  return addCodecSizePrefix(getUtf8Codec(), getU32Codec2());
}
var tokenMetadataCodec = getStructCodec2([
  ["updateAuthority", fixCodecSize(getBytesCodec2(), 32)],
  ["mint", fixCodecSize(getBytesCodec2(), 32)],
  ["name", getStringCodec()],
